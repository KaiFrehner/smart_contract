...
\node (start) [startstop, left=0.5cm of init] {trigger(current\_entry)};
\node (balance) [process, left=0.5cm of start] {Read contract balance};
\node (loop) [process, left=0.5cm of balance] {For each policy};

\node (before) [decision, below=1.5cm of loop] {Now \textless{} startTime};
\node (notactive) [process, below of= before] {Policy is not yet active};
\node (nostart) [process, below of = notactive] {Do nothing};

\node (active) [decision, right=2cm of before] {startTime \textless{} Now \textless{} endTime?};
\node (rain) [decision, right = 2.15cm of nostart ] {current\_entry \textgreater{}  rainfallAmt?};
\node (increment) [process, below =2.5cm of rain] {Increment daysOverThreshold \\ Record affected policy};

\node (expired) [decision, right=3cm of active] {Now \textgreater{} endTime?};
\node (endmark) [process, below of=expired] {Mark as ended};

\node (qualify) [decision, below of=endmark] {daysOverThreshold \\ \textgreater{}  deductibleDays?};
\node (payoutcalc) [process, below of=qualify] {Compute payout};
\node (funds) [decision, below of=payoutcalc] {Enough balance?};
\node (send) [process, below of=funds] {Payment};
\node (emitcov1) [process, below of=send] {Emit CoverageTriggered};

\node (emitcov2) [process, left = 0.5cm of send] {Emit CoverageTriggered \\ (no payout)};

\node (emitweather) [process, below of=increment, yshift=-2cm] {Emit addWeatherData event \\ (if affectedPolicies $\neq$ empty)};

\node (end) [startstop, below=0.5cm of emitweather] {End};
...
\draw [arrow] (before) -- node[right]{No} (notactive);
\draw [arrow] (notactive) -- (nostart);
...
\draw [arrow] (funds.west) -- ++(0,0) -| (emitcov2.north) node[midway,above]{No};
