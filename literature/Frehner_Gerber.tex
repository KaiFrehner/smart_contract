...
% Komentar: wieso macht man einen parametrische Wetterversicherung in einem Smart Contract?
...
\paragraph*{Time restriction}
The time restriction in the implemented smart contract is the same as the time restriction used in the Baloise insurance. The starting and end times are checked using the require function in Solidity. For testing purposes the minimum wait time of 14 days has been removed.
...
\paragraph*{Pricing}
Due to the secrecy around the pricing of the Baloise product, the premium calculation is considered out of scope for this project. In order to test the smart contract a simplified and rudimentary formula is used to obtain a premium dependent on the \textbf{duration} of insured trip, the amount of  \textbf{coverage}, the amount of \textbf{deductible days} and a rudimentary estimation of the \textbf{likelihood of rain in the insured area}. Additionally, a \textbf{safety margin}  $\Phi = \frac{11}{10}$ is added. 


\[
\text{premium} = \text{coverage} \ (\text{duration} - \text{thresholdForRegion}) \   \mu_X \  \Phi
\]
...
\begin{figure}[h] % [h] = "here", direkt an dieser Stelle
    ...
    \caption{example of a triggered parametric weather insurance given that travel occurred between days 252 and 260 with deductible days of less than 4. The data is the of sum of precipitation between the hours of 8 AM and 6 PM per day. The precipitation data is from Berlin in September 2025.}
...
This section describes the implementation of the parametric weather insurance using a smart contract. 
Some of the features outlined above are simplified, either because they are out of scope for this project or because they proved impractical for testing purposes. 

The Basic structure of the smart contract is, that the insurer issues you the contract, and the insured can buy a policy on the smart contract. The information of the policies (i.e. policy holder, region traveled to, amount of deductible days, threshold for bad weather (2.9mm)) is stored within the smart contract. The aggregated weather data for the observed time slot is imported once daily and triggers the function to check with policies are triggered. This function is outlined more closely below.

\paragraph*{Input Checks} 
As the contract requires user interaction to set up a policy, the inputs need to be checked. This is primarily done with the \texttt{require} function in solidity. This way it is assured that user input is usable and that the premium amount is paid. 

\paragraph*{Events}
For key events such as state changes in the smart contract events are triggered. These events are then stored in the logs of the smart contract. In this smart contract events are triggered when a new policy is set up, when new weather information is published to the smart contract and which policies are affected by the new weather data point. Finally it is logged when a policy ends and a payment is made.

\paragraph*{Data Import}
In order to receive external data into the smart contract an oracle is used. A blockchain oracle is an external service that supplies off-chain data—such as weather measurements—to a smart contract, enabling it to react to real-world events that the blockchain cannot observe natively. As noted by Antonopoulos and Wood (2018), oracles are essential for connecting deterministic on-chain logic with unpredictable off-chain information.
In this implementation the weather data is 

In a final implementation the daily aggregated 

\paragraph*{Outline of Trigger Function} \label{sec:trigger}
