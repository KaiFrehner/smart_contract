...
\node (start) [startstop, below of=init] {trigger(current\_entry)};
\node (balance) [process, below of=start] {Read contract balance};
\node (loop) [process, below of=balance] {For each policy};

\node (before) [decision, below of=loop] {Now \textless{} startTime};
...
\node (rain) [decision, right = 2cm of nostart ] {current\_entry \textgreater{}  rainfallAmt?};
