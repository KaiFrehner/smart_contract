...
\captionof{Spain: infrequent rainfall}
\end{column}
...
\subsection{Pricing} 
\begin{frame}
\frametitle{Simplified pricing formula} 
\footnotesize
\begin{itemize}
    \item Information on pricing formula is not available from Baloise
    \item a simplified approach is used
\end{itemize}
...
\vspace{0.2cm}
\begin{itemize}
    \item coverage: amount paid out per bad-weather day
    \item $D_{\text{tot}}$: Total duration of the trip
    \item $D_{\text{ded}}$: Deductible days for the specific travel destination
    \item $\mu_X$: Chance of rain in the specific travel destination
     \item $\Phi$:  safety margin of 1.1 is included to account for uncertainty
 \end{itemize}
...
\begin{frame}
\frametitle{Implementation with Solidity}
\begin{itemize}
    \item Insurer deploys the smart contract and funds it with initial capital.
    \item Travellers purchase policies directly on-chain.
    \item Each policy stores the holder, region, timeframe, and rainfall threshold.
    \item Daily weather data is supplied through an oracle.
    \item The \texttt{trigger()} function:
    \begin{itemize}
        \item updates policies based on new rainfall data,
        \item tracks bad-weather days,
        \item emits events for transparency,
        \item processes payouts once trips have ended.
    \end{itemize}
    \item All logic and events remain verifiable on the blockchain.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Trip duration: \textbf{ 9 Sep 2025$-$17 Sep 2025 } (8 days)
    \item Berlin has on average \textbf{65 rainy days/year} with precipitation $\ge 2.9$ mm
    \item $\Rightarrow$ Risk category: \textbf{C} \rightarrow $\mu_X = 0.3$
    \item Deductible days: $\text{round}(8 \times 0.3) = 2$
    \item Payout is triggered starting from the \textbf{3rd rainy day}
    \item Coverage amount = \textbf{50 EUR/day}
    \item Premium:
     \[
         \text{EUR 50} \ \cdot \ (8 - 2) \ \cdot \ 0.3 \ \cdot \ 1.1
        = \textbf{99 EUR}
    \]
    \item Payout: $ \text{EUR 50} \ \cdot \ (4 - 2) 
        = \textbf{100 EUR}$
\end{itemize}
...
\includegraphics[width=\linewidth,keepaspectratio]{weather_2025_Sep_pres}
\vspace{0.5em} % adjust space as needed
\includegraphics[width=\linewidth,height=0.45\textheight,keepaspectratio]{berlin}
\end{column}
...
\bibitem[ECA&D]{paket}
\emph{ECA&D}. \text{https://www.ecad.eu/}

\bibitem[Wetterheld]{paket}
\emph{Wetterheld}. \text{https://wetterheld.com/home/de/}

\bibitem[Swiss Re]{paket}
\emph{Swiss Re: Parametric Insurance Solutions}. \text{https://corporatesolutions.swissre.com/}
\text{alternative-risk-transfer/parametric-solutions.html}
